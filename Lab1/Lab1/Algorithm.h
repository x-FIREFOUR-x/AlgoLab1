...
class Algorithm
{
	struct State_puzzle
	{
		...
		const bool operator<( State_puzzle) { return true; }
	};
...
private:
	...
	*/
	...
	void search(priority_queue <pair<int, State_puzzle>, vector<pair<int, State_puzzle>>, greater<pair<int, State_puzzle>>> &pri_queue, bool& is_search, vector<State_puzzle>& states, vector<Puzzle>& used);
	void build_all_son_bfs(priority_queue <pair<int, State_puzzle>, vector<pair<int, State_puzzle>>, greater<pair<int, State_puzzle>>> &pri_queue, State_puzzle current_state, vector<State_puzzle>& states, int index_father, vector<Puzzle>& used);
	void build_son_bfs(priority_queue <pair<int, State_puzzle>, vector<pair<int, State_puzzle>>, greater<pair<int, State_puzzle>>> &pri_queue, Puzzle p, int index_father, vector<State_puzzle>& states, vector<Puzzle>& used);
